...
export default defineConfig({
  ...
  layout: {
    ...
    // ✅ menuDataRender nhận menuData gốc, filter theo role
    menuDataRender: () => {
      let role = '';
      try {
        if (typeof window !== 'undefined') {
          const raw = window.localStorage.getItem('ript_user');
        ...
        return [
          {
            ...
            routes: [
              { path: '/admin/requests', name: 'Quản lý yêu cầu', icon: 'FileOutlined' },
              { path: '/admin/devices', name: 'Quản lý thiết bị', icon: 'ToolOutlined' },
              { path: '/admin/stats', name: 'Thống kê', icon: 'BarChartOutlined' },
            ],
          },
        ];
      }
        ...
        return [
          {
            ...
            icon: 'UserOutlined',
            routes: [
              { path: '/student/dashboard', name: 'Dashboard', icon: 'HomeOutlined' },
              { path: '/student/devices', name: 'Danh sách thiết bị', icon: 'LaptopOutlined' },
              { path: '/student/history', name: 'Lịch sử mượn', icon: 'HistoryOutlined' },
            ],
          },
        ];
      }
