...
router.post('/:id/approve', async (req, res) => {
    ...
    if (requestRow.status !== 'pending')
        return res.status(400).json({ error: 'Request must be pending' });
    const equipmentResult = await db_1.default.query('SELECT quantity FROM equipments WHERE id=$1', [requestRow.equipment_id]);
    if (!equipmentResult.rows.length)
        return res.status(404).json({ error: 'Equipment not found' });
    const quantity = equipmentResult.rows[0].quantity;
    if (quantity < requestRow.amount)
...
router.post('/:id/return', async (req, res) => {
    ...
    await db_1.default.query('UPDATE requests SET status=$1, updated_at=now() WHERE id=$2', ['returned', id]);
    await (0, equipment_1.adjust)(requestRow.equipment_id, requestRow.amount);
    res.json({ ok: true });
});
