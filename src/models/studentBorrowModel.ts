...
export default {
	...
	reducers: {
		save(state: StudentBorrowModelState, { payload }: { payload: Partial<StudentBorrowModelState> }) {
			return {
				...state,
				...payload,
			};
	...
	effects: {
		*login({ payload }: { payload: { email: string; password: string } }, { put }: any) {
		...
		*submitBorrowRequest({ payload }: { payload: BorrowRequest }, { put }: any) {
